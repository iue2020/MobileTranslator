...
class Firebase {
  ...
  parse = snapshot => {
    ...
    var myid = this.current().uid;

    if(nowtimestamp - timestamp < 600 && myid == user.receiverId)
    {
      //Trans.shared.translateMe(message.text);
      var str = Trans.shared.translateMe(message.text);
      //var str2 = Trans.shared.translateMe2(message.text);
      //Tts.shared.readText(message.text),
      //Tts.shared.readText(Trans.shared.translateMe(message.text)),
      console.log("işte bu")
    }
    return message;
  };
  ...
  setMoLang=(key) => 
    {
        Trans.shared.setMyLanguage(key);
    }
  ...
  on = async (callback, id) => {
    ...
    firebase
      ...
      .on('child_added', snapshot => callback(this.parse(snapshot)), function(snapshot){
